...
extern void GetEventServiceCapabilities(struct soap* soap, struct tev__Capabilities* capabilities);
...
SOAP_FMAC5 int SOAP_FMAC6 __tds__GetServices(struct soap* soap, struct _tds__GetServices *tds__GetServices, struct _tds__GetServicesResponse *tds__GetServicesResponse) {
	...
	for(int i = 0; i < tds__GetServicesResponse->__sizeService; i++) {
		...
		if (i == 0) {
			...
			tds__GetServicesResponse->Service[i].Capabilities->tev__Capabilities = (struct tev__Capabilities*)soap_malloc(soap, sizeof(struct tev__Capabilities));
			memset(tds__GetServicesResponse->Service[i].Capabilities->tev__Capabilities, 0, sizeof(struct tev__Capabilities));
			GetEventServiceCapabilities(soap, tds__GetServicesResponse->Service[i].Capabilities->tev__Capabilities);
		} else if (i == 4) {
